      ...
      <div className="flex items-center justify-between pointer-events-auto w-full gap-4">
        ...
        <nav className={`hidden lg:flex items-center gap-8 bg-[#ffffff]/95 backdrop-blur-xl rounded-full px-8 py-3.5 shadow-[0_10px_30px_rgba(0,0,0,0.12)] border border-gray-200/50 transition-all duration-300 ${
          ...
          >
            <button
              className={`text-xs uppercase tracking-wider font-extrabold transition-colors duration-300 flex items-center gap-1 hover:text-gold cursor-pointer py-1 ${
                pathname === "/portfolio" || pathname === "/investor"
                  ? "text-gold"
                  : "text-[#121212]"
              }`}
            >
          ...
          <Link
            href="/blog"
            className={`relative text-xs uppercase tracking-wider font-extrabold transition-colors duration-300 hover:text-gold py-1 group/nav-link ${
              pathname === "/blog" ? "text-gold" : "text-[#121212]"
            }`}
          >
            Media
            <span className={`absolute bottom-0 left-0 w-0 h-0.5 bg-gold transition-all duration-300 group-hover/nav-link:w-full ${pathname === "/blog" ? "w-full" : ""}`} />
          </Link>

          <Link
            href="/events"
            className={`relative text-xs uppercase tracking-wider font-extrabold transition-colors duration-300 hover:text-gold py-1 group/nav-link ${
              pathname === "/events" ? "text-gold" : "text-[#121212]"
            }`}
          >
            Cohort Events
            <span className={`absolute bottom-0 left-0 w-0 h-0.5 bg-gold transition-all duration-300 group-hover/nav-link:w-full ${pathname === "/events" ? "w-full" : ""}`} />
          </Link>

          <Link
            href="/download"
            className={`relative text-xs uppercase tracking-wider font-extrabold transition-colors duration-300 hover:text-gold py-1 group/nav-link ${
              pathname === "/download" ? "text-gold" : "text-[#121212]"
            }`}
          >
            Profile
            <span className={`absolute bottom-0 left-0 w-0 h-0.5 bg-gold transition-all duration-300 group-hover/nav-link:w-full ${pathname === "/download" ? "w-full" : ""}`} />
          </Link>
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-4">
              <Link
                href="/apply?mode=membership"
                onClick={() => setIsOpen(false)}
                className="w-full py-3.5 text-center text-xs font-bold uppercase tracking-wider text-white bg-cta-gradient rounded-full shadow-lg"
              >
                For Club Membership
              </Link>
            </div>
